@if ($componentType === 'edit')
    <div id="titleComponent" class="text-lg font-bold flex items-center justify-between w-full px-4 space-x-2 relative" style="height: 130px;" >
        <div class="flex items-center space-x-2" style="display: block;">
            <img src="{{ asset('icons/new-item.svg') }}" alt="Upload Icon" style="height: 30px; padding-right: 10px; padding-left:50px; display: inline-block;">
            <div style="display: inline-grid;">
                <span style="color: #141824; font-size: 24px; font-weight: 600; line-height: 28.6px; text-align: left;">{{$slot}}</span>
                @if($titleDescription)
                    <div style="color:#8A94AD; font-size: 20px; font-weight: 600; line-height: 28.6px; text-align: left;">{{$titleDescription}}</div>
                @endif
            </div>
