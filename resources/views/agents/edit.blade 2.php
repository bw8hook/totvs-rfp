            ...
            <div class="list-form" style="padding-top:0px;">
                ...
                <div id="titleComponentForm" class=" flex items-center justify-between px-4 space-x-2 relative">
                    <div class="flex items-center space-x-2" style="display: block; margin-left: 30px;">
                        ...
                        <div style="display: inline-grid; width: 94%;">
                            <span style="color: #141824; font-size: 22px; font-weight: 600; line-height: 28.6px; text-align: left;">Editar Agentes</span>
                            <div style="color:#8A94AD; font-size: 16px; font-weight: 400; line-height: 20px; text-align: left; margin-top: 8px; ">Preencha o formulário abaixo para editar um Agente já cadastrada. Certifique-se de fornecer todas as informações necessárias.</div>
                        </div>
                    </div>
                </div>

                <form method="POST" action="{{ route('agents.update', $agents->id) }}" enctype="multipart/form-data" style=" margin: auto; width: 65%; min-width: 450px; background: #FFF; box-shadow: 0px 4px 28px rgba(0, 0, 0, 0.1); border-radius: 15px; padding: 50px 15%;">
                @csrf
                
                <!-- Name -->
                <div>
                    <x-input-label for="name" :value="__('Nome')" />
                    <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" value="" required autofocus autocomplete="name"  value="{{ old('name', $agents->agent_name) }}" />
                    <x-input-error :messages="$errors->get('name')" class="mt-2" />
                </div>      
                
                <!-- Name -->
                <!-- <div>
                    <x-input-label for="knowledge_id" :value="__('Base de Conhecimento Mentor IA')" />
                    <x-text-input id="knowledge_id" class="block mt-1 w-full" type="text" name="knowledge_id" value="" disabled autofocus value="{{ old('knowledge_id', $agents->knowledge_id) }}" />
                    <x-input-error :messages="$errors->get('knowledge_id')" class="mt-2" />
                </div>     

               
                <div>
                    <x-input-label for="knowledge_id_hook" :value="__('Base de Conhecimento HOOK IA')" />
                    <x-text-input id="knowledge_id_hook" class="block mt-1 w-full" type="text" name="knowledge_id_hook" value=""  disabled autofocus value="{{ old('knowledge_id_hook', $agents->knowledge_id_hook) }}" />
                    <x-input-error :messages="$errors->get('knowledge_id_hook')" class="mt-2" />
                </div>     -->

                 <!-- search_engine -->
                <div class="mt-4">
                    <x-input-label for="search_engine" :value="__('IA Engine')" />
                    <select name="search_engine"  class="form-control">
                        <option value="Mentor IA"  {{ $agents->search_engine === 'Mentor IA' ? 'selected' : '' }} >Mentor IA</option>
                       <option value="Open IA"  {{ $agents->search_engine === 'Open IA' ? 'selected' : '' }} >Open IA</option>
                    </select>
                    <x-input-error :messages="$errors->get('search_engine')" class="mt-2" />
                </div>
               ...
               <div class="mt-4">
                    ...
                    <select name="status"  class="form-control">
                        <option value="ativo"  {{ $agents->status === 'ativo' ? 'selected' : '' }} >Ativo</option>
                       <option value="inativo"  {{ $agents->status === 'inativo' ? 'selected' : '' }} >Inativo</option>
                    </select>
