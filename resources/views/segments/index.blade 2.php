        ...
        <div class="max-w-full mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div id="titleComponent" class="text-lg font-bold flex items-center justify-between w-full px-4 space-x-2 relative">
                ...
                <div class="relative flex items-center">        
                    <a href="{{ route('segments.create') }}" class="btn_titleComponent" >Cadastrar novo segmento </a>
                </div>
            </div>
                ...
                @if($Itens->isEmpty())
                    ...
                    <div>
                        ...
                        <div style="width: 40%; margin: auto;text-align: center;font-weight: 500;">
                            ...
                            <a href="{{ route('segments.create') }}" class="btn_titleComponent" style="margin: 20px 0; display: inline-table;" >Cadastrar novo segmento </a>
                        </div>
...
<script>
    $(document).ready(function () {
        function fetchUsers(url = "{{ route('segments.filter') }}") {
            $.ajax({
                url: url,
                method: 'GET',
                        ...
                        rows += `
                            <tr class="listaTabela" style=" min-height: 60px; max-height: 100%; ">
                              <td style="width:5%; display: flex;">#${role.id}</td>
                                <td style="width:75%; text-align:left; display: flex;">${role.name}</td>
                                ...
                                <td style="width:10%; text-align:center; position:relative; display:flex;">
                                    <a class="btn_edit_row" href="/segments/${role.id}" style="margin: -6px 10px;;">
                                    ...
                                    <div class="btn_delete_row">
                                        <form action="segments/${role.id}" method="POST" onsubmit="return confirm('Tem certeza que deseja excluir este item?');" style="margin: 0px;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit">
