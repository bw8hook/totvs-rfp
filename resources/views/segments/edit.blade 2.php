            ...
            <div class="list-form" style="padding-top:0px;">
                ...
                <div id="titleComponentForm" class=" flex items-center justify-between px-4 space-x-2 relative">
                    <div class="flex items-center space-x-2" style="display: block; margin-left: 30px;">
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display: inline; margin-top: 3px; width: 27px; margin-right: 1px;">
                            <path d="M21.21 15.89A10 10 0 1 1 8 2.83" style="color: #5570F1;"></path>
                            <path d="M22 12A10 10 0 0 0 12 2v10z" style="color: #5570F1;"></path>
                        </svg>


                        <div style="display: inline-grid; width: 94%;">
                            <span style="color: #141824; font-size: 22px; font-weight: 600; line-height: 28.6px; text-align: left;">Editar Segmento</span>
                            <div style="color:#8A94AD; font-size: 16px; font-weight: 400; line-height: 20px; text-align: left; margin-top: 8px; ">Preencha o formulário abaixo para editar um Segmento já cadastrado. Certifique-se de fornecer todas as informações necessárias.</div>
                        </div>
                    </div>
                </div>

                <form method="POST" action="{{ route('segments.update', $Segment->id) }}" enctype="multipart/form-data" style=" margin: auto; width: 65%; min-width: 450px; background: #FFF; box-shadow: 0px 4px 28px rgba(0, 0, 0, 0.1); border-radius: 15px; padding: 50px 15%;">
                @csrf
                
                <!-- Name -->
                <div>
                    <x-input-label for="name" :value="__('Nome')" />
                    <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" value="" required autofocus autocomplete="name"  value="{{ old('name', $Segment->name) }}" />
               ...
               <div class="mt-4">
                    ...
                    <select name="status"  class="form-control">
                        <option value="ativo"  {{ $Segment->status === 'ativo' ? 'selected' : '' }} >Ativo</option>
                       <option value="inativo"  {{ $Segment->status === 'inativo' ? 'selected' : '' }} >Inativo</option>
                    </select>
