            ...
            <div class="list-form" style="padding-top:0px;">
                ...
                <div id="titleComponentForm" class=" flex items-center justify-between px-4 space-x-2 relative">
                    <div class="flex items-center space-x-2" style="display: block; margin-left: 30px;">
                        ...
                        <div style="display: inline-grid; width: 94%;">
                            <span style="color: #141824; font-size: 22px; font-weight: 600; line-height: 28.6px; text-align: left;">Editar Processo</span>
                            <div style="color:#8A94AD; font-size: 16px; font-weight: 400; line-height: 20px; text-align: left; margin-top: 8px; ">Preencha o formulário abaixo para editar um produto já cadastrado. Certifique-se de fornecer todas as informações necessárias.</div>
                        </div>
                    </div>
                </div>

                <form method="POST" action="{{ route('process.update', $process->id) }}" enctype="multipart/form-data" style=" margin: auto; width: 65%; min-width: 450px; background: #FFF; box-shadow: 0px 4px 28px rgba(0, 0, 0, 0.1); border-radius: 15px; padding: 50px 15%;">
                @csrf
                
                <!-- Name -->
                <div>
                    <x-input-label for="process" :value="__('Nome')" />
                    <x-text-input id="process" class="block mt-1 w-full" type="text" name="process" value="" required autofocus autocomplete="name"  value="{{ old('name', $process->process) }}" />
               ...
               <div class="mt-4">
                    ...
                    <select name="status"  class="form-control">
                        <option value="ativo"  {{ $process->status === 'ativo' ? 'selected' : '' }} >Ativo</option>
                       <option value="inativo"  {{ $process->status === 'inativo' ? 'selected' : '' }} >Inativo</option>
                    </select>
