<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        @if (Auth::user()->role == "admin")
            ...
            <div class="mt-4">
                <x-input-label for="agent" :value="__('Agente')" />
            ...
            <div class="mt-4">
                <x-input-label for="base" :value="__('Base de Conhecimento')" />
                <select name="base" id="base" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">
                    @foreach($listaBases as $Base)
                        @if ($Base->base_id == $BaseId)
                            <option selected value="{{$Base->base_id}}">{{$Base->description}}</option>
                        @else
