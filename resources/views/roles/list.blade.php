<x-app-layout>
    <div class="py-4" style=" padding-bottom: 130px;">
        <div class="max-w-full mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <x-title-component :showButton="true" textButton="Adicionar Perfil" urlButton="roles/new" > {{ __('Perfis Cadastrados') }} </x-title-component>

            <div id="BlocoLista" class="p-4 sm:p-8">
                <div class="bloco_info_details_header">
                    <div id="bloco_info_found">
                        <div class="info_details_total">
                            Total Encontrados: <span></span>
                        </div>
                    </div>
                ...
                <table id="userTable" class="tabela">
                    <thead>
                        <tr>
                            ...
                            <th style="width:35%;">Permissões</th>
...
<script>
    $(document).ready(function () {
        function fetchUsers(url = "{{ route('roles.filter') }}") {
            $.ajax({
                url: url,
                method: 'GET',
                ...
                success: function (response) {
                    ...
                    response.data.forEach(role => {
                        ...
                        let ListPermissions = role.permissions;
                        let PermissionsList = '';
        
                        ListPermissions.forEach(permissions => {
                        ...
                        rows += `
                            <tr class="listaTabela" style=" min-height: 60px; max-height: 100%; ">
                              <td style="width:5%; display: flex;">#${role.id}</td>
                                <td style="width:21%; text-align:left; display: flex;">${role.name}</td>
                                <td style="width:51%; text-align:left; ">${PermissionsList}</td>
                                <td style="width:17%; text-align:center; display: flex;">${formattedDate}</td>
                                <td style="width:10%; text-align:center; position:relative; display:flex;">
                                    <a class="btn_edit_row" href="/roles/edit/${role.id}" style="margin: -6px 10px;;">
