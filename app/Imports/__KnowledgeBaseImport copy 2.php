        ...
        try {
            ...
            foreach ($rows as $index => $row) {  
                ...
                try {
                    ...
                    $IdInserted = $KnowledgeRecord->save();
                   ...
                   dd($KnowledgeRecord);
                    ...
                    if (!$bundleIDFound) {
                        try {
                            $inserted = DB::table('knowledge_records_bundles')->insert([
                                'knowledge_record_id' => $IdInserted,
                                'bundle_id' => null,
                                'old_bundle' => $row[5],
                            ...
                            \Log::error('Erro na inserção: ' . $e->getMessage());
                            dd([
                                'erro' => $e->getMessage(),
                                'knowledge_record_id' => $IdInserted,
                                'old_bundle' => $row[5]
                    ...
                    foreach ($ProdutosAdicionais as $ProdutoAdicional) {
                     ...
                     // Divide o texto por vírgulas e limpa espaços
                    $Tipos = array_map('trim', explode(',', $row[8]));
                    foreach ($Tipos as $Tipo) {
                         // Procura o bundle pelo nome
                         $typeExist = Type::where('name', 'like', '%' . $Tipo . '%')->first();
                         if ($typeExist) {
                             // Se encontrou o bundle, vincula ao knowledge record
                             $KnowledgeRecord->types()->attach($typeExist->id, [
                                 'old_type' => $Tipo,
                             ]);
                         } else {
                             // Se não encontrou o bundle, adiciona como type
                            DB::table('knowledge_records_types')->insert([
                                'knowledge_record_id' => $IdInserted,
                                'type_id' => null,
                                'old_type' => $Tipo,
                                'created_at' => now(),
